...
| 🌊 **Organic drift** | A checkbox that sets interior vertices swaying back and forth, each on its own random speed, amplitude and phase, scaled to the mesh's own cell size. Corners and edges stay fixed, so a mesh needs at least 2 × 2 patches for it to do anything. |
| 🌈 **Color drift** | A checkbox that sets every vertex's hue swinging independently around the color it was given — saturation, brightness and alpha stay put, so turning it off always lands back on the picked color. |
